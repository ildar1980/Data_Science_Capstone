...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.RangeSlider(id='payload-slider', min=0, max=20000, step=1000,
...
def get_pie_chart(entered_site, slider_range):
    if entered_site == 'ALL':
        ...
        fig1 = px.pie(filtered_df,
                     names='class',
                     title='Total Success Launch by {entered_site}')
        df = filtered_df[(filtered_df['Payload Mass (kg)'] > slider_range[0]) & (filtered_df['Payload Mass (kg)'] < slider_range[1])]
        fig2 = px.scatter(df, x="Payload Mass (kg)", y="class", color="Booster Version Category")
        return fig1, fig2
